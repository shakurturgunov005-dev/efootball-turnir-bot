...
async def check_payments(message: types.Message):
    ...
    async with db.pool.acquire() as conn:
        rows = await conn.fetch("SELECT * FROM players WHERE payment_status = FALSE AND payment_photo IS NOT NULL")
    if not rows:
    ...
    for row in rows:
        ...
        await message.answer(f"💰 **Yangi to'lov**\n\n👤 {row['full_name']}\n🆔 {row['user_id']}", reply_markup=keyboard, parse_mode="Markdown")
        if row['payment_photo']:
            await message.bot.send_photo(message.chat.id, row['payment_photo'], caption="📸 To'lov cheki")
...
async def confirm_payment(callback: types.CallbackQuery):
    ...
    try:
        await callback.bot.send_message(user_id, f"✅ **Tabriklaymiz!**\n\nTo'lovingiz tasdiqlandi.\nSiz {count}-ishtirokchi bo'ldingiz!", parse_mode="Markdown")
    except:
        pass
...
async def send_post(message: types.Message):
    ...
    await message.bot.send_message(chat_id=CHANNEL_ID, text=text, reply_markup=register_btn, parse_mode="Markdown")
...
async def clear_all(message: types.Message):
    ...
    await message.answer("⚠️ **Barcha ma'lumotlarni tozalashni xohlaysizmi?**", reply_markup=keyboard, parse_mode="Markdown")
