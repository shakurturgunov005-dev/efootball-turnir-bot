...
async def edit_list(message: types.Message):
    ...
    for player in players:
        ...
        text = f"{status} {player['full_name']} | @{player['telegram_username']}"
            ...
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="👁 Ko'rish",
                        callback_data=f"view_{player['user_id']}"
                    ),
                    InlineKeyboardButton(
                        text="✅ Tasdiqlash",
                        callback_data=f"pay_{player['user_id']}"
                    ),
                    InlineKeyboardButton(
                        text="❌ O'chirish",
                        callback_data=f"del_{player['user_id']}"
                    )
                ]
            ]
        )
